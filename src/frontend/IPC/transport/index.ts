...
export function setTransport(api: BackendTransport) {
    try {
        window.api = api
        activeTransport = api
    } catch (err) {
...
export function installTransport(transport?: BackendTransport) {
    if (transport) {
    ...
    if (isWebBuild()) {
        ...
        hadToken = !!token
        setTransport(createSocketApi({ auth: token ? { token } : undefined, onStatus, onUnauthorized: handleUnauthorized }))
        return
    }
    ...
    if (electronApi && window.api !== electronApi) setTransport(electronApi)
}
